...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if args.prompt:
        tmux.send_keys(f"{session}:0.0", args.prompt, "Enter")
...
def _status_payload(identifier: str) -> dict[str, object]:
    ...
    try:
        ...
        return {"identifier": identifier, "session": session, "exists": False, "done": False, "capture": ""}
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    for session in sorted(name for name in live if name.startswith("cc-tmux-") and name not in seen):
        payload.append({"session": session, "project_path": None, "created_at": None, "exists": True})
    if args.json:
        _print_json(payload)
    else:
        if not payload:
        ...
        for item in payload:
            print(f"{item['session']}\t{'live' if item['exists'] else 'stopped'}\t{item.get('project_path') or ''}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--claude-arg", action="append", default=[], help="extra argument passed to claude; repeatable")
    p.add_argument("--auto-trust", action=argparse.BooleanOptionalAction, default=True)
    p.add_argument("--startup-delay", type=float, default=1.0, help=argparse.SUPPRESS)
    ...
    p.add_argument("--fallback-kill", action="store_true", help="kill if /exit does not stop within --wait")
